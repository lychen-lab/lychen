 ...
 */
final readonly class AreaRequestPatchProcessor implements ProcessorInterface
{
    /**
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): AreaRequest
    {
        ...
        \assert($entity instanceof \App\Entity\AreaRequest);

        // Map standard fields
        $entity = $this->objectMapper->map($data, $entity);

        // Handle activities relation
        if ($data instanceof AreaRequestPatch && null !== $data->activities) {
